...
- 优化前端组件的响应式数据管理，减少不必要的重渲染
- 改进图表初始化逻辑，复用图表实例避免重复创建
- 优化前端代理配置，确保API请求正确路由
...
### v1.2
- 优化前端代理配置，修复登录页面加载错误
- 优化Vue3 Composition API使用，减少组件重渲染
- 改进ECharts图表初始化逻辑，复用图表实例
- 添加前端mock数据，确保页面在权限受限情况下正常显示
- 更新数据库连接配置，确保使用正确的连接信息
- 完善README.md文档，更新项目最新状态
